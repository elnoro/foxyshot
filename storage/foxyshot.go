...
func NewUploader(c *config.Config) (Uploader, error) {
	if c.Foxyshot.Address != "" {
		return newFoxishotUploader(c.Foxyshot.Address, c.Foxyshot.Insecure)
	}

	return NewS3Uploader(c.S3), nil
}

func newFoxishotUploader(address string, insecure bool) (Uploader, error) {
	...
	return &FoxyshotStorage{client: grpcClient}, nil
}
...
func (fs *FoxyshotStorage) Upload(ctx context.Context, path string, options *UploadOptions) (string, error) {
	...
	l, err := fs.client.Upload(ctx, screenshot)
	if err != nil {
		return "", err
	}
